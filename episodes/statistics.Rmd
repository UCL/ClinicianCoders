...
Contingency tables of frequences can also be tabulated with **table()**. For example:
...
::: notes
Tell them to do this. The default for t.test() is that variances are unequal and it produces Wech's statistic.
:::
...
Use the **lm()** function to model the relatinship between cchic$creatininegroup and cchic$ph_abg.  Compare the results with the ANOVA carried out earlier.
